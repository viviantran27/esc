...
%% load exp data
name = 'Cell_43';
cell = readtable(['DCR data\DCR_' name '_CA2.txt']); 
% cell = readtable("DCR_Cell_1_CA2.txt"); %change filename in pulse_plotting_simulink.m

% stackedplot(data)
cell(1:468, :) =[]; %cell(1:498, :) =[];
cell.time_s = cell.time_s - cell.time_s(1);
cell.Time = cell.time_s;
cell.x_Q_Qo__mA_h = cell.x_Q_Qo__mA_h - cell.x_Q_Qo__mA_h(1);
cell.I = -cell.I_mA/1000; 
cell.Voltage = cell.Ecell_V;
%% Find period markers
current=cell.I;
...
tau = R1.*C1;
sortrows(table(SOC_RC, Rs, R1, C1, tau))
...
% Runs pulse simulation with new parameters. Need to add model to path
